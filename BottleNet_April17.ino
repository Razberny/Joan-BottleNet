...
void loop() {
  ...
  if (systemActive) {
    ...
    digitalWrite(ssrPin, LOW);  // Turn shredder OFF only when inactive
...
bool waitForFinalVerification(unsigned long timeout = 5000) {
  unsigned long startTime = millis();
  while (millis() - startTime < timeout) {
    if (digitalRead(FINAL_IR_PIN) == LOW) {
      return true;  // IR sensor triggered — bottle passed through
    }
  }
  return false;  // Timed out — bottle didn’t pass
}
...
void inspectBottle() {
  ...
  lcd.print("Waiting...");
  lcd.setCursor(0, 1);
  lcd.print("Insert Bottle Now");

  bool confirmed = waitForFinalVerification(); // wait up to 5 seconds
