  ...
  const float BIN_FULL_THRESHOLD = 1.0; // cm or less means bin is full
  ...
  void loop() {
    ...
    if (bottleDetected()) {
      ...
      // 👇 Add this block after inspecting bottle
      if ((millis() - lastActivityTime) > INACTIVITY_TIMEOUT) {
        systemActive = false;
  ...
  void shredder() {
    ...
    // 10-second countdown on lower right (position 13,1)
    for (int i = 10; i > 0; i--) {
      lcd.setCursor(13, 1);  // Right side of 2nd row (13-15 columns)
